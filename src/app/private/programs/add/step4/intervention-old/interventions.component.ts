...
export class InterventionsComponent implements OnInit, AfterViewInit {
  ...
  createIntervention(intervention: HTMLInterventionElement, graphIndex: number) {
    const interventionComponent: ComponentRef<InterventionComponent> = this.interventionsContainer.createComponent(
      this.componentFactoryResolver.resolveComponentFactory(InterventionComponent)
    );
